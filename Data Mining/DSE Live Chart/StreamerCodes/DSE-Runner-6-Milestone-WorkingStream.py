...
#     hours="%I",           # Format hours as 12-hour clock without leading zero
#     minutes="%H:%M:%S",   # Format minutes and seconds as HH:MM:SS
#     # am_pm="%p"            # Format AM/PM indicator
# )
...
p, l1 = draw_shape_line(x_col_name='x', y_col_name='y1', source=source, figure=p, label='Testy time', shape='c')
...
legend = Legend(items=[l1, legend_item], location="top_left", orientation="vertical")
...
def update_data():
        ...
        # time.sleep(1)
        # tmp_y1 = get_co_data(co_name='BEXIMCO')['percentage_change']
        # time.sleep(random.randint(5, 10))
        # tmp_y2 = get_co_data(co_name='AAMRATECH')['percentage_change']
        new_data = dict(
        x=[datetime.now()],
...
curdoc().add_periodic_callback(update_data, 3000)  # 180000
